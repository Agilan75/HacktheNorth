...
/**
 * The page reads `GET /rules` and `GET /verification` live. jsdom has neither,
 * so `useApi` is mocked here and fed a small real-shaped payload: enough to
 * prove the weight matrix is derived from the response rather than typed into
 * the page.
 */
interface FakeState {
...
let rulesState: FakeState;
let verificationState: FakeState;

vi.mock('../api/useApi.js', () => ({
  useApi: (select: (client: Record<string, () => Promise<unknown>>) => Promise<unknown>) => {
    ...
    void select(probe);
    if (asked === 'getVerification') return verificationState;
    if (asked === 'getRules') return rulesState;
    /*
     * `#brief` reads the queue and the rank-1 account. This page's tests are
     * about the dossier, not about that section, so both come back empty and
     * the section renders its no-evidence form. Anything unrecognised lands
     * here too, rather than being handed another endpoint's payload.
     */
    return { data: null, loading: true, error: null, reload: () => undefined };
  },
  useApiClient: () => ({}),
}));
...
describe('TourPage story', () => {
  it('has no WebGL under jsdom, and is a plain readable document without it', () => {
    ...
    renderPage();
    expect(
      screen.getByRole('heading', { level: 1, name: /Pan your camera around your room/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('heading', { level: 2, name: 'Gemini sees and labels objects.' }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('heading', { level: 2, name: 'The engine deterministically decides.' }),
    ).toBeInTheDocument();
  });
  ...
  it('keeps exactly one h1, and labels every beat', () => {
    ...
    // All seven are shot from inside the apartment, ending on `out`. The rail
    // keeps stops past that; the story no longer uses them.
    expect(beats).toHaveLength(7);
    for (const beat of beats) expect(beat).toHaveAttribute('aria-labelledby');
  });
...
describe('TourPage dossier', () => {
  it('indexes every section and renders each as a region', () => {
    renderPage();
    ...
    // The finale: the brief, answered, last in the dossier and in the index.
    expect(index.getByRole('link', { name: 'The brief, answered' })).toHaveAttribute('href', '#brief');
  ...
  it('collapses every panel except the deliberate ones, and opens them all on demand', async () => {
    ...
    const panels = [...container.querySelectorAll<HTMLDetailsElement>('.rf-tour__panel')];
    ...
    openAll.click();
    const inSection = [...section.querySelectorAll<HTMLDetailsElement>('details')];
  ...
  it('leaves the agent section a readable document with no WebGL', () => {
    ...
    const steps = [...agent.querySelectorAll('.rf-tour__panel > summary')].map((s) =>
      s.textContent?.trim(),
    );
  ...
  it('opens each agent step without a scene to drive', () => {
    ...
    const agent = screen.getByRole('region', { name: 'The Federato agent' });
    const panels = [...agent.querySelectorAll<HTMLDetailsElement>('.rf-tour__panel')];
  ...
  it('a deep link into the dossier skips the film instead of being swallowed by it', () => {
    // Under jsdom there is no WebGL, so the film never runs and the guarantee
    // below is really about the anchors: /tour#brief must point at a section
    // that exists, which is what makes the deep link worth honouring at all.
    renderPage();
    const index = within(screen.getByRole('navigation', { name: 'Sections' }));
    for (const href of ['#rules', '#agent', '#testing', '#brief']) {
      const chip = index.getAllByRole('link').find((a) => a.getAttribute('href') === href);
      expect(chip, `no chip for ${href}`).toBeDefined();
  ...
  it('ends on the ways into the console', () => {
    renderPage();
    // Scoped to the "Open it" section: the brief section above it also links to
    // Verification from its ledger, which is a second link with the same name.
    const open = screen.getByRole('region', { name: 'Open it' });
    expect(within(open).getByRole('link', { name: 'The ranked book' })).toHaveAttribute('href', '/queue');
    expect(within(open).getByRole('link', { name: 'Appetite terrain in 3D' })).toHaveAttribute('href', '/explore');
    expect(within(open).getByRole('link', { name: 'Verification' })).toHaveAttribute('href', '/verification');
  });
});
