...
const citation = (section: string, quote: string) => ({
...
const interpretations: readonly unknown[] = [
  {
    id: 'I-2',
...
function Probe(): ReactElement {
  ...
  return <span data-testid="search">{lastSearch}</span>;
}
...
describe('RulesPage criteria matrix', () => {
  ...
  it('states each criterion as a threshold, not as a raw condition', () => {
    ...
    expect(within(book).getByRole('button', { name: /AG-TIV-T/ })).toHaveTextContent(
      '$50.0M–$100.0M',
    );
  ...
  it('opens the rule named in the URL on arrival, so a verdict can link straight at it', () => {
    apiState = ready();
    render(
      <MemoryRouter initialEntries={['/rules?rule=AG-TIV-N']}>
        <RulesPage />
      </MemoryRouter>,
    );
