...
import type { PipelineParts } from './tour-pipeline.js';
...
export const ANCHOR_IDS = ['heater', 'curtain', 'bed', 'detector', 'desk', 'fix'] as const;
...
const WINDOWS: Readonly<Record<AnchorId, readonly [number, number]>> = {
  heater: [0.06, 0.44],
  curtain: [0.26, 0.46],
  bed: [0.26, 0.46],
  detector: [0.42, 0.6],
...
function flat(color: string, roughness = 0.92): THREE.MeshStandardMaterial {
 ...
 */
const EYES: readonly THREE.Vector3[] = [
...
const LOOKS: readonly THREE.Vector3[] = [
...
const FOG_ROOM: readonly [number, number] = [12, 26];
/** Fog once out on the pipeline, which is ~90 units long end to end. */
const FOG_PIPE: readonly [number, number] = [70, 300];
/** Pipeline-local progress over which the fog opens up. */
const FOG_OPEN: readonly [number, number] = [0.0, 0.35];

/** 0 outside the window, 1 inside it, eased across a `fade`-wide lip at each edge. */
function windowOpacity(t: number, [from, to]: readonly [number, number], fade = 0.06): number {
...
interface Built {
  ...
  readonly heater: THREE.Group;
  readonly anchors: Readonly<Record<AnchorId, THREE.Vector3>>;
  readonly materials: readonly THREE.Material[];
  readonly geometries: readonly THREE.BufferGeometry[];
}

function buildRoom(): Built {
  ...
  const track = (o: THREE.Object3D): void => {
    o.traverse((child) => {
  ...
  const paneMat = keep(
    new THREE.MeshStandardMaterial({ color: MAT.glow, emissive: new THREE.Color(MAT.glow), emissiveIntensity: 0.45, roughness: 1 }),
  );
...
export function createHouseScene(host: HTMLElement, reducedMotion: boolean): HouseScene {
  ...
  const pipeline: PipelineParts = buildPipeline();
  scene.add(pipeline.root);
  const started = performance.now();

  const badges = new Map<AnchorId, HTMLElement>();
  ...
  function tick(): void {
    ...
    if (current > ROOM_SPAN - 0.02) needsRender = true;
  ...
  return {
    ...
    dispose(): void {
      ...
      for (const m of pipeline.materials) m.dispose();
      for (const g of pipeline.geometries) g.dispose();
      renderer.dispose();
