...
const SCREEN_SIZE = { w: 4.6, h: 2.2 } as const;
const SCREEN_ROWS = 5;
/** Score-bar length as a fraction of the row's track, top row first. */
const SCREEN_BARS = [0.94, 0.78, 0.6, 0.41, 0.24] as const;

/** Phone handset: body, screen, one verdict chip, a price bar, two fix rows. */
const PHONE_BODY = { w: 1.5, h: 2.7 } as const;
const PHONE_BEZEL = 0.16;
/** Fix-step row lengths as a fraction of the screen width. */
const PHONE_FIX_ROWS = [0.82, 0.55] as const;

/** Ramp windows. `ramp(t, a, b)` is a smoothstep from a to b. */
const W_ROOT_VISIBLE = 0.02;
const W_APARTMENT: readonly [number, number] = [0.05, 0.3];
const W_FEDERATO: readonly [number, number] = [0.13, 0.35];
const W_STREAM: readonly [number, number] = [0.14, 0.4];
const W_SHELL: readonly [number, number] = [0.3, 0.55];
const W_EDGE_LABEL: readonly [number, number] = [0.33, 0.5];
const W_CORE: readonly [number, number] = [0.42, 0.62];
const W_OUTPUT: readonly [number, number] = [0.7, 0.88];
const W_DETAIL: readonly [number, number] = [0.8, 0.96];
...
const STREAM_SPEED = [0.115, 0.095] as const;
...
const RING_SPIN = [0.031, -0.044, 0.038, -0.028, 0.051, -0.035] as const;
/** Fixed tilts, so six near-identical radii still read as six rings. */
const RING_TILT_X = [0.06, -0.14, 0.22, -0.27, 0.12, -0.33] as const;
const RING_TILT_Z = [-0.2, 0.11, -0.08, 0.24, -0.3, 0.16] as const;
 ...
 */
function makeLabelCanvas(): LabelCanvas | null {
  ...
  if (typeof OffscreenCanvas !== 'undefined') return new OffscreenCanvas(LABEL_W, LABEL_H);
  return null;
}
...
function paintLabel(ctx: Ctx2D, text: string, style: LabelStyle): void {
 ...
 */
export function pipelineLabel(text: string, opts: LabelStyle = {}): THREE.Sprite {
  const mat = new THREE.SpriteMaterial({ transparent: true, depthWrite: false, opacity: 1 });
  const canvas = makeLabelCanvas();
  const ctx = canvas === null ? null : (canvas.getContext('2d') as Ctx2D | null);
...
/** One material whose opacity is a plain ramp over its own window of `t`. */
interface Fade {
  readonly m: THREE.Material;
  readonly a: number;
  readonly b: number;
...
interface Stream {
  ...
  readonly rgb: THREE.Color;
}

export function buildPipeline(): PipelineParts {
  ...
  const fade = <T extends THREE.Material>(m: T, a: number, b: number, max: number): T => {
    m.transparent = true;
    m.opacity = 0;
  ...
  function label(text: string, worldWidth: number, style: LabelStyle): THREE.Sprite {
  ...
  function volume(
    ...
    const fillMat = fade(
      new THREE.MeshBasicMaterial({ color: COLORS.paper, depthWrite: false }),
      a,
      b,
      0.62,
    );
  ...
  const apartmentGeo = keepGeo(
    new THREE.BoxGeometry(APARTMENT_SIZE.x, APARTMENT_SIZE.y, APARTMENT_SIZE.z),
  );
  const apartmentEdges = keepGeo(new THREE.EdgesGeometry(apartmentGeo));
  const apartmentMat = fade(
  ...
  const coreFill = fade(
    new THREE.MeshBasicMaterial({ color: BLUE_TINT, depthWrite: false }),
    W_CORE[0],
    W_CORE[1],
    0.55,
  );
  ...
  for (let i = 0; i < 6; i += 1) {
    ...
    const group = new THREE.Group();
    const geo = keepGeo(
      new THREE.TorusGeometry(r, RING_TUBE, RING_RADIAL_SEGMENTS, RING_TUBULAR_SEGMENTS),
    );
    const mat = keep(
      new THREE.MeshBasicMaterial({ color: COLORS.muted, transparent: true, opacity: 0, depthWrite: false }),
    );
  ...
  function plate(
    ...
    const mat = fade(
      new THREE.MeshBasicMaterial({ color, depthWrite: false, side: THREE.DoubleSide }),
      W_DETAIL[0],
      W_DETAIL[1],
      max,
    );
  ...
  function consoleGlyph(): THREE.Group {
  ...
  function phoneGlyph(): THREE.Group {
    const g = new THREE.Group();
  ...
  function output(pos: THREE.Vector3, name: string, glyph: THREE.Group): THREE.Group {
  ...
  const cards: Card[] = cardSpecs.map((spec, i) => {
    ...
    return {
      ...
      target: new THREE.Vector3(
        spec.target.x - 5.6,
        spec.target.y + 1.2 + (i % 2) * 2.2,
        spec.target.z + 5.0 + i * 1.2,
      ),
  ...
  function update(t: number, elapsed: number): void {
    ...
    for (let s = 0; s < streams.length; s += 1) {
      ...
      const pos = stream.position.array as Float32Array;
      const col = stream.color.array as Float32Array;
    ...
    for (let i = 0; i < cards.length; i += 1) {
      ...
      card.sprite.position.set(
        scratch.x,
        scratch.y + CARD_BOB * Math.sin(elapsed * 1.1 + card.phase) * (1 - travel * 0.6),
        scratch.z,
      );
