...
import type { SubmissionDetailView } from '../panels/types.js';

const h = vi.hoisted(() => ({ state: { data: null as unknown, loading: false, error: null as Error | null, reload: () => {} }, client: {} as Record<string, unknown>, lastId: '' }));

vi.mock('../api/useApi.js', () => ({
  useApiClient: () => h.client,
  useApi: (select: (c: unknown) => Promise<unknown>, deps: readonly unknown[]) => { h.lastId = String(deps[0]); return h.state; },
}));
const panel = vi.hoisted(() => (name: string) => (p: Record<string, unknown>) => <div data-testid={name}>{JSON.stringify(p, (_k, v) => typeof v === 'function' ? 'fn' : v)}</div>);
...
vi.mock('../panels/Actions.js', () => ({ Actions: (p: { onApprove: (id: string) => Promise<void>; routing: { rationale: string } }) => <button data-rationale={p.routing.rationale} onClick={() => void p.onApprove('act-1')}>approve</button> }));
vi.mock('../panels/ReplyBox.js', () => ({ ReplyBox: (p: { pending: boolean; result: unknown; onSubmitText: (t: string) => Promise<void> }) => <div data-testid="k-reply" data-pending={String(p.pending)}>{JSON.stringify(p.result)}<button onClick={() => void p.onSubmitText('sprinklered: yes')}>send</button></div> }));
vi.mock('../panels/AttachedSweep.js', () => ({ AttachedSweep: panel('l') }));

import { SubmissionPage } from './SubmissionPage.js';

function detail(over: Partial<SubmissionDetailView> = {}): SubmissionDetailView {
  const pricing = { quotedPremium: 41250, predictedPremium: 50000, adequacy: 0.825, expectedLoss: 12000, ratePer100Tiv: 0.31, currency: 'USD' as const, factors: [], notes: [] };
  return {
...
function mount(path: string | { pathname: string; state: unknown } = '/submissions/SUB-7') {
  const entry = typeof path === 'string' ? path : { pathname: path.pathname, state: path.state };
  return render(<MemoryRouter initialEntries={[entry]}><Routes><Route path="/submissions/:id" element={<SubmissionPage />} /><Route path="/queue" element={<p>queue</p>} /></Routes></MemoryRouter>);
}
...
describe('SubmissionPage', () => {
  it('leads with the decision, then groups the twelve panels under three plain sections', () => {
    ...
    for (const l of 'abcdefghijkl') expect(document.getElementById('panel-' + l)).not.toBeNull();
  });
...
function knockout(over: Partial<SubmissionDetailView> = {}): SubmissionDetailView {
  const base = detail();
  return detail({
...
function noPolicyDetail(over: Partial<SubmissionDetailView> = {}): SubmissionDetailView {
  return detail({
...
const VERIFICATION = {
  ...
  engine: { verdict: 'REFER' as const, appetiteScore: 81, knockoutFactorIds: [], decidingFactorId: 'building_age' },
  matchesCurrentResult: true,
  naive: { verdict: 'REFER' as const, appetiteScore: 81, knockoutFactorIds: [], decidingFactorId: 'building_age', agrees: { verdict: true, appetiteScore: true, knockouts: true, decidingFactor: true, all: true } },
  secondOpinion: null,
};
