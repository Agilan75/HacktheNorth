...
function cite(factorLabel: string, quote: string): CitationView {
...
type TierCell = readonly [tier: TierLabel, tierValue: number] | null;

/** Pre-computed points, written as literals so the fixture never does arithmetic. */
const POINTS: Readonly<Record<string, Readonly<Record<string, number>>>> = {
  '0.1': { '1': 10, '0.6': 6, '0': 0 },
  '0.15': { '1': 15, '0.6': 9, '0': 0 },
};

function factorRows(cells: readonly TierCell[], knockoutFactor: string | null, referFactor: string | null): FactorRowView[] {
  return FACTORS.map((f, i) => {
    ...
    const tierForRow: TierLabel = f.factorId === referFactor ? 'refer' : tier;
    return {
      factorId: f.factorId,
      label: f.label,
      tier: tierForRow,
...
const T: TierCell = ['target', 1];
const A: TierCell = ['acceptable', 0.6];
const N: TierCell = ['not_acceptable', 0];

/** B1 tiers: 1, 1, 1, 0.6, 0.6, 0.6, 1, 1. */
const B1_CELLS: readonly TierCell[] = [T, T, T, A, A, A, T, T];
/** B2: as B1 but `tiv` -> 0. */
const B2_CELLS: readonly TierCell[] = [T, T, T, N, A, A, T, T];
/** No policy: only line of business is known. */
const NO_POLICY_CELLS: readonly TierCell[] = [null, T, null, null, null, null, null, null];

const COMMERCIAL_SPEC: readonly (readonly [string, string, boolean, boolean])[] = [
...
function vectorComponents(rows: readonly (readonly [raw: number | null, tier: number | null])[]): VectorComponentView[] {
...
function queryTrace(root: 'Policy' | 'Submission', rows: number): QueryTraceEntryView[] {
  return [
    {
      step: 1,
      phase: 'discover',
...
const ENRICHMENT: readonly EnrichmentCardView[] = [
  {
    source: 'openfema',
...
function logEntry(
  ...
  type: string,
  status: string,
  before: readonly [score: number, rank: number, verdict: Verdict] | null,
  after: readonly [score: number, rank: number, verdict: Verdict] | null,
): ActionLogEntryView {
  return {
    actionId,
    submissionId,
...
function fit(): SubmissionDetailView {
  const id = 'sub-fit';
...
function refer(): SubmissionDetailView {
  const base = fit();
  const id = 'sub-refer';
...
function dnf(): SubmissionDetailView {
  const base = fit();
  const id = 'sub-dnf';
...
function noPolicy(): SubmissionDetailView {
 ...
 */
export function fixtureQueue(): readonly QueueRowView[] {
  const row = (
    s: SubmissionDetailView,
    rank: number,
    qualityIndex: number,
    extra: Partial<QueueRowView> = {},
  ): QueueRowView => ({
