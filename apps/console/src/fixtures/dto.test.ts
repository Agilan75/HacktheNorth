...
import type { SubmissionDetailView } from '../panels/types';
import { fixtureQueue, fixtureSubmission } from './dto';

const KINDS = ['fit', 'refer', 'dnf', 'no-policy'] as const;
...
function quality(s: SubmissionDetailView): number {
  const p = s.pricing;
  ...
  if (p.adequacy !== null) terms.push([0.2, (Math.min(Math.max(p.adequacy, 0), 2) / 2) * 100]);
...
describe('fixtureSubmission', () => {
  it.each(KINDS)('%s: points are 100·w·t and sum to the appetite score', (kind) => {
    ...
    for (const f of s.factors) {
      ...
      if (!f.known) expect(f.tierValue).toBeNull();
    }
  ...
  it.each(KINDS)('%s: vector masks agree with raw/tier nulls and the pricing ratios', (kind) => {
    ...
    for (const c of s.vector.components) {
      ...
      if (c.mask === 0) expect(c.tier).toBeNull();
    }
    const p = s.pricing;
