...
function fakeFetch(routes: Record<string, unknown | ((call: Call) => { status: number; body: unknown })>) {
  const calls: Call[] = [];
  const impl = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    const { status, body: out } =
      typeof handler === 'function' ? (handler as (c: Call) => { status: number; body: unknown })(call) : { status: 200, body: handler };
...
function snapshot(appetiteScore: number, verdict: string, rank: number) {
...
function queueRow(id: string, rank: number, extra: Record<string, unknown> = {}) {
  return {
...
describe('createApiClient', () => {
  ...
  it('throws the ErrorDto message on a non-2xx answer', async () => {
    ...
    const err = await createApiClient({ baseUrl: BASE, fetchImpl: impl }).getSubmission('nope').catch((e: unknown) => e);
    expect(err).toBeInstanceOf(Error);
    expect((err as Error).message).toBe('no submission "nope"');
