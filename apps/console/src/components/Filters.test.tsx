...
import type { FilterOptions, QueueFilterValue } from './Filters';

const empty: QueueFilterValue = { line: null, verdict: null, state: null, underwriter: null, search: '' };
...
describe('Filters', () => {
  it('renders a labelled search landmark with every PRD §10 filter', () => {
    render(<Filters value={empty} options={options} onChange={() => {}} />);
  ...
  it('emits the full next value on each change, with null for "All"', () => {
    ...
    const value: QueueFilterValue = { ...empty, state: 'TX' };
    render(<Filters value={value} options={options} onChange={onChange} />);
  ...
  it('reflects the current value and clears everything', () => {
    const onChange = vi.fn();
    const value: QueueFilterValue = { line: 'Auto', verdict: 'REFER', state: 'NY', underwriter: 'Riley', search: 'x' };
    render(<Filters value={value} options={options} onChange={onChange} />);
  ...
  it('keeps a selected value visible even when it is missing from the options', () => {
    render(<Filters value={{ ...empty, state: 'ON' }} options={options} onChange={() => {}} />);
