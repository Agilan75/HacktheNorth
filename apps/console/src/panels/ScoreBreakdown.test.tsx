...
import type { FactorRowView, TierLabel } from './types.js';

afterEach(() => cleanup());

/** INTERPRETATIONS W-1 weights, in factor order. */
const WEIGHTS: readonly [string, string, number][] = [
...
function tierLabel(t: number): TierLabel {
  if (t === 1) return 'target';
  if (t === 0.6) return 'acceptable';
  return 'not_acceptable';
}

/** Rows as the engine emits them: points = 100 × weight × tierValue (engine FiredRule). */
function rows(tiers: readonly (number | null)[]): FactorRowView[] {
...
function rowFor(factorId: string): HTMLElement {
  return screen.getByTestId(`points-${factorId}`).closest('tr') as HTMLElement;
}

describe('ScoreBreakdown', () => {
  it('B1: renders all eight factors with W-1 weights and 100·w·t points, total 84.0', () => {
    render(<ScoreBreakdown factors={rows(B1)} appetiteScore={84} completeness={100} />);
    expect(screen.getByRole('region', { name: 'Score breakdown' }).id).toBe('b');
    expect(screen.getAllByRole('row')).toHaveLength(1 + 8);

    const expectedPoints: Record<string, string> = {
  ...
  it('B11: a missing premium scores 0, says Missing, and the total is the 81.0 prop', () => {
    const tiers = [...B1];
    tiers[4] = null as unknown as number;
    render(
      <ScoreBreakdown factors={rows(tiers)} appetiteScore={81} completeness={(8 / 9) * 100} />,
    );
  ...
  it('B12: a knockout is labelled in words and does not zero the 86.0 score', () => {
    ...
    render(<ScoreBreakdown factors={rows(tiers)} appetiteScore={86} completeness={100} />);
  ...
  it('prints the score prop, never a re-sum of the rows', () => {
    ...
    render(<ScoreBreakdown factors={rows(B1)} appetiteScore={84.5} completeness={100} />);
    expect(screen.getByTestId('breakdown-score').textContent).toBe('84.5/100');
  });

  it('shows the empty label with no factors', () => {
    render(<ScoreBreakdown factors={[]} appetiteScore={0} completeness={0} />);
