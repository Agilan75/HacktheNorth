...
import type { ExplanationView } from './types.js';

afterEach(() => cleanup());

function view(overrides: Partial<ExplanationView> = {}): ExplanationView {
  return {
    verdict: 'REFER',
...
describe('Explanation', () => {
  it('renders headline, non-empty paragraphs, recommendation and panel anchor', () => {
    const { container } = render(
      <Explanation explanation={view()} verdict="REFER" appetiteScore={81} />,
    );
    expect(screen.getByRole('region', { name: 'Explanation and recommendation' }).id).toBe('a');
    expect(screen.getByTestId('explanation-headline').textContent).toBe(
    ...
    expect(screen.getByTestId('explanation-recommendation').textContent).toBe(
      'Investigate: request the quoted premium from the broker.',
    );
  });

  it('prints the score, confidence and deciding rule from props without recomputing', () => {
    render(<Explanation explanation={view()} verdict="REFER" appetiteScore={81} />);
    ...
    expect(screen.getByTestId('explanation-deciding-rule').textContent).toBe(
      'AG-building_age-acceptable',
    );
  });
  ...
  it('B1 FIT at 84.0 with confidence 1 and no deciding rule', () => {
    const { container } = render(
      <Explanation
        explanation={view({ verdict: 'FIT', decidingRuleId: null, confidence: 1 })}
        verdict="FIT"
        appetiteScore={84}
      />,
    );
  ...
  it('V-8 (c): no deciding factor when every appetite factor is missing', () => {
    const { container } = render(
      <Explanation
        explanation={view({ decidingFactorId: null, decidingRuleId: null, recommendation: '  ' })}
        verdict="DOES_NOT_FIT"
        appetiteScore={0}
      />,
    );
    expect(screen.getByTestId('explanation-deciding-factor').textContent).toBe(
      'None: every appetite factor is missing',
    );
