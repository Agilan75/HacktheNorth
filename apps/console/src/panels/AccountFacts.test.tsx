...
const KNOCKOUT_FACTS: SubmissionFactsView = {
...
describe('AccountFacts', () => {
  it('a triage knockout says why in words, with the guideline quote, and lists every fact', () => {
    render(<AccountFacts accountKind="triage_knockout" displayLineOfBusiness="health" facts={KNOCKOUT_FACTS} factors={[LOB_NA]} traceAnchor="panel-c" />);
  ...
  it('a no-policy account names what was assessed and what is missing, from the factor rows', () => {
    ...
    render(
      <AccountFacts
        ...
        facts={{ ...KNOCKOUT_FACTS, lineOfBusiness: 'property', status: 'declined', declineReason: 'loss_history', underwriterName: null }}
        factors={factors}
        traceAnchor="panel-c"
      />,
    );
  ...
  it('facts stored before they were read say so, and every value reads "Not read yet"', () => {
    const empty: SubmissionFactsView = {
    ...
    render(<AccountFacts accountKind="triage_knockout" displayLineOfBusiness="cyber" facts={empty} factors={[]} traceAnchor="panel-c" />);
  ...
  it('an API without facts still names the line and says the facts are not sent', () => {
    render(<AccountFacts accountKind="triage_knockout" displayLineOfBusiness="auto" facts={null} factors={[]} traceAnchor="panel-c" />);
