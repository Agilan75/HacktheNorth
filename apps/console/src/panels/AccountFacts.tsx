...
import type { CitationView, FactorRowView, SubmissionFactsView } from './types.js';
 ...
 */
export function lineOfBusinessLabel(line: string | null | undefined): string {
  if (typeof line !== 'string' || line.trim().length === 0) return 'Not recorded';
  const trimmed = line.trim();
  if (/^[a-z]{1,3}$/i.test(trimmed)) return trimmed.toUpperCase();
...
const reasonHeadStyle: CSSProperties = {
  margin: 0,
...
const absentStyle: CSSProperties = { color: cssVar('muted-deep'), fontStyle: 'italic' };
...
function absentText(facts: SubmissionFactsView | null, whenNull: string): string {
  if (facts === null || facts.source === 'not_fetched') return 'Not read yet';
  return whenNull;
}
...
function factRows(props: AccountFactsProps): readonly FactRow[] {
  const f = props.facts;
  const missing = (text: string): string => absentText(f, text);
...
function WhyNotScored(props: AccountFactsProps): ReactElement {
  if (props.accountKind === 'triage_knockout') {
    ...
    const citation: CitationView | null = lob?.citation ?? null;
    return (
      <div style={reasonStyle} data-testid="why-not-scored" data-kind="triage_knockout">
        <p style={reasonHeadStyle}>{`Knocked out at triage: line of business is ${line}.`}</p>
        <p style={{ margin: `${SPACE.sm}px 0 0` }}>
          The carrier's appetite covers property only; every other line is Not Acceptable. Because of
          that, the agent never queried this submission in depth, so Federato's policy, buildings,
          premium and losses were never read and there is nothing further to score.
        </p>
        {citation !== null ? <CitationQuote citation={citation} compact /> : null}
      </div>
    );
  }
  ...
  return (
    <div style={reasonStyle} data-testid="why-not-scored" data-kind="no_policy">
      <p style={reasonHeadStyle}>Not fully scored: Federato holds no policy for this submission.</p>
      <p style={{ margin: `${SPACE.sm}px 0 0` }}>
        With no policy there are no buildings, premium or loss history to score.
 ...
 */
export function AccountFacts(props: AccountFactsProps): ReactElement {
  const { facts } = props;
  const rows = factRows(props);
  let provenance: ReactNode;
  if (facts === null) {
    ...
    provenance = (
      <>
        {'Read from Federato’s Submission record by the triage query'}
        {facts.traceId !== null ? (
          <>
            {' '}
            <a href={`#${props.traceAnchor}`}>{facts.traceId}</a>
          </>
        ) : null}
        {'. Display only: none of these values is scored.'}
      </>
    );
  }

  return (
    <div data-testid="account-facts">
      <WhyNotScored {...props} />
      <dl style={gridStyle} aria-label="Submission facts">
        {rows.map((r) => (
          <div key={r.key} data-fact={r.key}>
            <dt style={termStyle}>{r.term}</dt>
            <dd style={valueStyle}>{r.value !== null ? r.value : <span style={absentStyle}>{r.absent}</span>}</dd>
          </div>
        ))}
      </dl>
      <p style={noteStyle} data-testid="facts-provenance">
        {provenance}
      </p>
    </div>
  );
