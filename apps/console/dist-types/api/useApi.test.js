...
describe('useApiClient', () => {
    it('returns one shared client pointed at localhost:3000 by default', async () => {
        ...
        const fetchMock = vi.fn(async () => new Response(JSON.stringify({ ok: true, version: '0.1.0', adapter: 'live', llmConfigured: true, submissionCount: 1, startedAt: 'x' })));
        vi.stubGlobal('fetch', fetchMock);
        await expect(first.health()).resolves.toEqual({ ok: true, adapter: 'live', version: '0.1.0' });
        expect(String(fetchMock.mock.calls[0][0])).toBe('http://localhost:3000/health');
    });
});
describe('useApi', () => {
    it('loads, exposes data, and reloads on demand', async () => {
        let n = 0;
        const select = vi.fn(async (_client) => ++n);
    ...
    it('discards a stale response when deps change mid-flight', async () => {
        ...
        const { result, rerender } = renderHook(({ id }) => useApi(() => new Promise((resolve) => resolvers.set(id, resolve)), [id]), { initialProps: { id: 'a' } });
        rerender({ id: 'b' });
        await act(async () => {
            resolvers.get('b')?.('B');
        });
