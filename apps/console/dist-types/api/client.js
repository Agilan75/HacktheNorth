...
function makeTransport(options) {
    ...
    return async (route, params = {}, init = {}) => {
        const def = ROUTES[route];
        const url = `${base}${withQuery(routePath(route, params), init.query)}`;
        const headers = { accept: 'application/json' };
        const requestInit = { method: def.method, headers };
        if (def.method === 'POST') {
        ...
        if (text.length > 0) {
            try {
                ...
                throw new ApiError(response.status, 'BAD_JSON', `${def.method} ${def.path}: response was not JSON (HTTP ${response.status})`);
            }
        }
        if (!response.ok) {
            if (isErrorDto(json)) {
...
const TIER_LABELS = new Set(['target', 'acceptable', 'not_acceptable', 'refer']);
function tierLabel(tier) {
    return tier !== null && TIER_LABELS.has(tier) ? tier : null;
}
...
function queueRowView(row) {
    return {
        ...
        assignedUnderwriter: row.assignedUnderwriter?.name ?? row.federatoUnderwriter ?? null,
        underwriterSource: row.assignedUnderwriter !== null ? 'routed' : row.federatoUnderwriter != null ? 'federato' : null,
        totalTiv: row.totalTiv,
...
function schemaView(fieldMap) {
    ...
    for (const e of fieldMap.entries)
        bump(e.rawPath, true);
    for (const u of fieldMap.unmapped)
        bump(u.rawPath, false);
    return {
...
function buildingRows(dto) {
    return dto.buildings.map((b) => {
        const flags = [];
        if (b.pre1990)
            flags.push('pre-1990');
        if (b.post2010)
            flags.push('2010 or later');
        if (!b.acceptableConstruction)
            flags.push('unacceptable construction');
        if (b.assumedAcceptableConstruction)
            flags.push('construction assumed acceptable');
        const address = [b.name, b.city].filter((s) => typeof s === 'string' && s.length > 0);
        return {
            id: b.externalId,
...
function draftViews(dto) {
    const swing = new Map();
    for (const c of dto.result.voi.ranked)
        swing.set(c.question.field, c.expectedScoreSwing);
...
function actionLogEntry(a, before = a.before, after = a.after, rankBefore = a.rankBefore, rankAfter = a.rankAfter) {
    return {
        actionId: a.id,
...
function histogramBucket(index, total) {
    const lo = index * 10;
...
function aggregateView(dto, queue) {
    const byId = new Map(queue.map((r) => [r.submissionId, r]));
    const oneFlipAway = dto.oneFlipAway.map((o) => {
    ...
    return {
        ...
        oneFlipMoves: Object.fromEntries(dto.oneFlipAway.map((o) => [o.id, { moveLabel: o.moveLabel, scoreAfter: o.scoreAfter, premiumAfter: o.premiumAfter }])),
    };
}
...
export function createApiClient(options) {
    ...
    const getQueue = async () => {
        ...
        let offset = 0;
        for (;;) {
            const page = await call('listSubmissions', {}, {
                query: { limit: MAX_PAGE_LIMIT, offset },
            });
            rows.push(...page.rows);
            offset += page.rows.length;
            if (page.rows.length === 0 || offset >= page.page.total)
                break;
        }
        return rows.map(queueRowView);
    };
    const getSubmission = async (id) => submissionView(await call('getSubmission', { id }));
    const getActions = async () => {
        const actions = [];
        let offset = 0;
        for (;;) {
            const page = await call('listActions', {}, {
                query: { limit: MAX_PAGE_LIMIT, offset },
            });
            actions.push(...page.actions);
            offset += page.actions.length;
            if (page.actions.length === 0 || offset >= page.page.total)
                break;
        }
    ...
    return {
        ...
        async planActions() {
            const dto = await call('planActions', {}, { body: {} });
            return dto.actions.map((a) => actionLogEntry(a));
        },
        getActions,
        async approveAction(actionId) {
            const dto = await call('approveAction', { id: actionId });
        ...
        async postReply(id, input) {
            ...
            return replyView(await call('replyToSubmission', { id }, { body }));
        },
        async getAggregate() {
