 ...
 */
const BLANK_TARGET_KEYS = new Set([
    'isNewBusiness',
    'isPropertyLine',
    'pctTivAcceptableConstruction',
    'fiveYearLoss',
]);
function tierName(key, tier) {
    if (tier === 1)
        return BLANK_TARGET_KEYS.has(key) ? 'Acceptable' : 'Target';
    if (tier === 0.6)
        return 'Acceptable';
    if (tier === 0)
