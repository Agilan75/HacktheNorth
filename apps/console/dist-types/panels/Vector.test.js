...
function vector(rows, completeness) {
    const components = SPEC.map(([key, label, immovable, appetiteFactor], index) => {
...
describe('Vector', () => {
    ...
    it('F-2: immovable components 0,1,2,5,6,8,10 are labelled; 3,4,7,9 are movable', () => {
        ...
        for (const key of immovable)
            expect(screen.getByTestId(`vector-movable-${key}`)).toHaveTextContent('Immovable');
        for (const key of movable)
            expect(screen.getByTestId(`vector-movable-${key}`)).toHaveTextContent('Movable');
    ...
    it('no policy: missing components show mask 0, no tier, and V-6 completeness 11.1%', () => {
        render(_jsx(Vector, { vector: vector([[null, null], [1, 1]], 100 / 9) }));
