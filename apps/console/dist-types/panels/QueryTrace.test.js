...
describe('QueryTrace', () => {
    ...
    describe('R3-2: the reasoning reads as prose (PRD 7.5 step 6)', () => {
        ...
        it('shows the goal, the rules that needed it, the path and why, and the rejected alternatives', () => {
            render(_jsx(QueryTrace, { entries: [reasoned] }));
            const entry = screen.getByTestId('query-trace-entry');
            expect(within(entry).getByTestId('trace-goal')).toHaveTextContent('Goal: Fetch building TIV and construction for the property survivors');
            const needs = within(entry).getAllByTestId('trace-need');
            expect(needs).toHaveLength(2);
            expect(needs[0]).toHaveTextContent('Rule commercial.tiv (tiv) needs buildings.*.tiv: the TIV band rule scores total insured value');
            expect(needs[1]).toHaveTextContent('Rule commercial.construction needs buildings.*.constructionType: construction class');
            expect(within(entry).getByTestId('trace-path')).toHaveTextContent('Went to Policy → exposure_units → location because Policy carries premium, TIV and the buildings in one query.');
        ...
        it('names an unknown adaptation kind verbatim', () => {
            render(_jsx(QueryTrace, { entries: [{ ...reasoned, adaptation: 'widen_window' }] }));
        ...
        it('says plainly when no scoring rule needed the query and when nothing was rejected', () => {
            render(_jsx(QueryTrace, { entries: [{ ...reasoned, requiredBy: [], alternativesRejected: [] }] }));
        ...
        it('renders an error with a text label', () => {
            render(_jsx(QueryTrace, { entries: [{ ...reasoned, error: 'Deep pass failed [500]: boom' }] }));
