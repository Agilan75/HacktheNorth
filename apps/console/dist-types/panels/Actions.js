...
function DraftCard(props) {
    ...
    return (_jsxs("article", { className: "rf-draft", "data-testid": "actions-draft", "data-action-id": draft.actionId, children: [_jsxs("header", { className: "rf-draft__header", children: [_jsx("h4", { className: "rf-draft__subject", children: draft.subject }), _jsx(Badge, { label: titleCase(draft.status), tone: approvable ? 'attention' : 'quiet' })] }), _jsx("p", { className: "rf-draft__body", style: { whiteSpace: 'pre-wrap' }, children: draft.body }), draft.requestedFields.length > 0 ? (_jsxs("table", { className: "rf-table", "aria-label": "Requested fields", children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { scope: "col", children: "Field" }), _jsx("th", { scope: "col", children: "Path" }), _jsx("th", { scope: "col", children: "Value of information" })] }) }), _jsx("tbody", { children: draft.requestedFields.map((f) => (_jsxs("tr", { "data-testid": "draft-field", children: [_jsx("th", { scope: "row", children: f.label }), _jsx("td", { children: _jsx("code", { children: f.path }) }), _jsx("td", { children: `${formatScore(f.voi, { decimals: 1 })} pts` })] }, f.path))) })] })) : null, approvable ? (_jsx("button", { type: "button", className: "rf-button", onClick: () => void approve(), disabled: pending, "aria-label": `Approve request: ${draft.subject}`, children: pending ? 'Approving…' : 'Approve and mark sent' })) : null, _jsx("p", { className: "rf-footnote", children: "Nothing is emailed; approving marks the request sent." }), state.kind === 'error' ? (_jsx("p", { role: "alert", className: "rf-error", children: `Approve failed: ${state.message}` })) : null] }));
}
