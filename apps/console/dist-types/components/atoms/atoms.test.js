...
describe('tokens.css', () => {
    ...
    it('carries the PRD 13 values', () => {
        ...
        for (const size of [34, 28, 22, 17, 15, 13])
            expect(tokensCss).toContain(`${size}px;`);
    });
});
describe('base.css', () => {
    it('never declares a real shadow', () => {
        ...
        for (const s of shadows)
            expect(s).toMatch(/box-shadow:\s*(none|var\(--rf-shadow-none\));/);
    });
    ...
    it('paints each verdict with the fill @retrofit/design gives it', () => {
        ...
        for (const [verdict, token] of Object.entries(fill)) {
            expect(VERDICT_STYLES[verdict].fill).toBe(COLORS[token]);
        }
    });
});
describe('VerdictPill', () => {
    ...
    ])('%s renders its word, label, mark and variant', (verdict, cls, variant, word, label, mark) => {
        const { container } = render(_jsx(VerdictPill, { verdict: verdict }));
    ...
    it('adds detail after the verdict word without replacing it', () => {
        const { container } = render(_jsx(VerdictPill, { verdict: "REFER", detail: "Roof age" }));
        const text = container.textContent ?? '';
        expect(text.indexOf('REFER')).toBeLessThan(text.indexOf('Roof age'));
        expect(container.querySelector('.rf-pill').getAttribute('title')).toBe('Refer to underwriter: Roof age');
    });
    it('ignores blank detail', () => {
        const { container } = render(_jsx(VerdictPill, { verdict: "FIT", detail: "   " }));
        expect(container.querySelector('.rf-pill__detail')).toBeNull();
    });
});
describe('Badge', () => {
    it('renders the label with the default neutral tone', () => {
        render(_jsx(Badge, { label: "1 flip from FIT" }));
    ...
    it('applies tone and title', () => {
        render(_jsx(Badge, { label: "3 contradictions", tone: "attention", title: "Open contradictions" }));
...
describe('CitationQuote', () => {
    it('renders document, page, row and the exact quote', () => {
        const { container } = render(_jsx(CitationQuote, { citation: {
                document: 'Property Appetite Guide',
                page: 4,
                row: 'Construction',
                quote: 'Fire resistive construction is a target class.',
            } }));
        expect(container.querySelector('blockquote').textContent).toBe('“Fire resistive construction is a target class.”');
        expect(container.querySelector('cite').textContent).toBe('Property Appetite Guide · p. 4 · row Construction');
        expect(container.querySelector('.rf-citation--compact')).toBeNull();
    });
    it('omits missing page and row', () => {
        const { container } = render(_jsx(CitationQuote, { citation: { document: 'Rulebook', page: null, row: null, quote: 'x' } }));
        expect(container.querySelector('cite').textContent).toBe('Rulebook');
    });
    it('keeps page 0 when present', () => {
        const { container } = render(_jsx(CitationQuote, { citation: { document: 'Doc', page: 0, quote: 'x' } }));
    ...
    it('compact truncates long quotes but keeps the full text in the title', () => {
        ...
        const { container } = render(_jsx(CitationQuote, { compact: true, citation: { document: 'Doc', page: 2, quote: long } }));
    ...
    it('compact leaves short quotes whole with no title', () => {
        const { container } = render(_jsx(CitationQuote, { compact: true, citation: { document: 'Doc', quote: 'Short quote.' } }));
        const bq = container.querySelector('blockquote');
...
describe('Skeleton', () => {
    it('announces its label as a busy status with 3 lines by default', () => {
        render(_jsx(Skeleton, { label: "Loading queue" }));
    ...
    it('honours lines, clamps nonsense, and shortens the last line', () => {
        const { container, rerender } = render(_jsx(Skeleton, { label: "x", lines: 5, width: "240px" }));
        const lines = container.querySelectorAll('.rf-skeleton__line');
        expect(lines.length).toBe(5);
        expect(lines[4].style.width).toBe('60%');
        expect(lines[0].style.width).toBe('100%');
        expect(container.querySelector('.rf-skeleton').style.width).toBe('240px');
        rerender(_jsx(Skeleton, { label: "x", lines: 0 }));
        expect(container.querySelectorAll('.rf-skeleton__line').length).toBe(1);
        rerender(_jsx(Skeleton, { label: "x", lines: Number.NaN }));
...
describe('Card', () => {
    it('renders a labelled section with anchor id, aside and children', () => {
        render(_jsx(Card, { title: "Score breakdown", anchorId: "b", aside: _jsx(Badge, { label: "8 factors" }), children: _jsx("p", { children: "body" }) }));
    ...
    it('omits the aside and anchor when not given', () => {
        const { container } = render(_jsx(Card, { title: "Rules", children: "x" }));
...
//# sourceMappingURL=atoms.test.js.map