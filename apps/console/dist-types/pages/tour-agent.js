...
import { ADAPT, DEEP, DEEP_BRANCHES, DEEP_PATH, EDGES, EDGE_SAMPLES, NODES, NODE_BY_ID, NODE_IDS, POSES, RECORDED, SLOWEST_MS, STEP_IDS, STEP_STARTS, TIMELINE_SECONDS, TRIAGE, UNMAPPED, edgePointAt, edgePoints, stepAt, } from './tour-agent-graph.js';
/* -------------------------------------------------------------------------- */
/* Badges                                                                     */
/* -------------------------------------------------------------------------- */
/**
 ...
 */
export const BADGE_IDS = [...NODE_IDS, 'readout'];
/* -------------------------------------------------------------------------- */
/* Timing                                                                     */
/* -------------------------------------------------------------------------- */
...
function phase(age, from, to) {
    return to <= from ? (age >= to ? 1 : 0) : ease(clamp01((age - from) / (to - from)));
}
function line(kit, points, color, opacity, dashed = false) {
...
function disc(kit, radius, color, opacity) {
...
function ring(kit, inner, outer, color, opacity) {
...
export function createAgentScene(host, options) {
    ...
    const kit = { materials: [], geometries: [] };
    ...
    const billboards = [];
    const nodeFill = new Map();
    const nodeRing = new Map();
    const nodeMark = new Map();
    ...
    const buildingAt = NODE_BY_ID[UNMAPPED.missesOn].at;
    const probeStub = line(kit, [buildingAt.clone(), buildingAt.clone().add(new THREE.Vector3(2.6, -1.5, 0.4))], COLORS.muted, 0, true);
    root.add(probeStub);
    ...
    const rejectLine = line(kit, [NODE_BY_ID.policy.at.clone(), submissionAt.clone()], COLORS.muted, 0, true);
     ...
     */
    function strokePath(edges, t, opacity = 1) {
        ...
        for (let i = 0; i < edges.length; i += 1) {
            ...
            if (local > 0 && local < 1)
                head = { edge, u: local };
            else if (local >= 1 && i === edges.length - 1)
                head = { edge, u: 1 };
        }
        return head;
    }
    ...
    function clearMarks() {
        for (const m of nodeMark.values())
            setOpacity(m, 0);
    }
    function markNode(id, opacity) {
        const m = nodeMark.get(id);
        if (m)
            setOpacity(m, opacity);
    }
    function setPulse(edge, u, opacity) {
    ...
    function choreograph(step, age) {
        ...
        if (step === 'locate') {
            ...
            if (landed > 0)
                markNode('location', Math.max(0.55, landed));
        ...
        if (step === 'adapt') {
            ...
            const path = ['exposure-location', 'location-building'];
        ...
        for (const q of RECORDED) {
            ...
            if (q.pass === 'deep')
                setPulse(head?.edge ?? null, head?.u ?? 0, local < 1 ? 1 : 0);
        }
        for (const node of NODES)
            markNode(node.id, 0.5);
        badgeOpacity.set('readout', 1);
    ...
    function tick() {
        ...
        if (moving)
            currentIndex += reducedMotion ? delta : delta * 0.12;
        else
...
//# sourceMappingURL=tour-agent.js.map