...
export const DEEP_BRANCHES = ['policy-insured', 'policy-claim'];
/* -------------------------------------------------------------------------- */
/* The six steps                                                              */
/* -------------------------------------------------------------------------- */
/** One per `<Panel>` in `AgentGroup`, in the order they appear. */
export const STEP_IDS = ['read', 'locate', 'triage', 'deep', 'adapt', 'trace'];
/**
 ...
 */
export const TRIAGE = { total: 158, kept: 38, dropped: 120 };
/** The deep pass: one `Policy` query, all 27 property policies fully hydrated. */
export const DEEP = { policies: 27 };
/**
 ...
 */
export const UNMAPPED = { field: 'protection class', missesOn: 'building', foundOn: 'location' };
 ...
 */
export function edgeCurve(edge) {
...
export function edgePointAt(edge, u, out) {
    ...
    if (!a || !b)
        return out.copy(NODE_BY_ID[edge.to].at);
    return out.lerpVectors(a, b, span - i);
}
//# sourceMappingURL=tour-agent-graph.js.map