...
const xOf = (tiv) => axis(tiv, TIV_MIN, TIV_MAX);
const zOf = (premium) => axis(premium, PREM_MIN, PREM_MAX);
const yOf = (score) => (clamp(score, 0, 100) / 100) * Y_TOP;
...
export function cellScore(account, tivBand, premiumBand) {
    return (account.basePoints + 100 * account.tivWeight * tivBand.tierValue + 100 * account.premiumWeight * premiumBand.tierValue);
}
/** The engine's verdict ladder (verdict.ts V-1..V-5) applied to one piece of ground. */
export function cellVerdict(account, tivBand, premiumBand) {
    if (account.knockedOutElsewhere || tivBand.tierValue === 0 || premiumBand.tierValue === 0)
        return 'DOES_NOT_FIT';
    if (account.refersElsewhere)
        return 'REFER';
    return 'FIT';
}
...
export function createTerrainScene(host, callbacks) {
    ...
    const addLabel = (text, at, size, color) => {
        const sprite = textSprite(text, { size, color });
        sprite.position.copy(at);
        content.add(sprite);
    };
    const setData = (account, footprints, bands) => {
        ...
        for (const tivBand of bands.tiv) {
            ...
            for (const premiumBand of bands.premium) {
                ...
                addLabel(`${Math.round(score)}`, new THREE.Vector3((x0 + x1) / 2, height + 2.5, (z0 + z1) / 2), 20, verdict === 'DOES_NOT_FIT' ? COLORS.muted : COLORS.ink);
            }
        }
        /* Axis labels at the band edges, each naming the rule that cuts there. */
        for (const band of bands.tiv) {
            if (!Number.isFinite(band.from))
                continue;
        ...
        for (const f of footprints) {
            ...
            const dot = new THREE.Mesh(new THREE.CircleGeometry(selected ? 1.6 : 1.1, 20), new THREE.MeshBasicMaterial({
        ...
        if (offMap.length > 0) {
            ...
            offMap.forEach((f, i) => {
                ...
                const selected = f.submissionId === account.submissionId;
                const dot = new THREE.Mesh(new THREE.CircleGeometry(selected ? 1.5 : 1, 16), new THREE.MeshBasicMaterial({
                    color: selected ? COLORS.red : dotColor(f),
                    transparent: true,
            ...
            addLabel(`Off the floor: no TIV or quoted premium (${offMap.length})${detail}`, new THREE.Vector3(originX + 16, 0.5, originZ - 9), 26, COLORS.mutedDeep);
        }
        ...
        if (account.tiv !== null && account.premium !== null) {
            ...
            const top = tivBand !== null && premiumBand !== null ? yOf(cellScore(account, tivBand, premiumBand)) : yOf(account.appetiteScore);
            const stalkTop = top + 16;
            content.add(line([new THREE.Vector3(x, top, z), new THREE.Vector3(x, stalkTop, z)], COLORS.ink, 0.8));
            const cap = new THREE.Mesh(new THREE.SphereGeometry(2.8, 24, 16), new THREE.MeshStandardMaterial({
            ...
            addLabel(`${account.insuredName} · ${Math.round(account.appetiteScore)}`, new THREE.Vector3(x, stalkTop + 6, z), 26, COLORS.ink);
            ...
            if (!account.knockedOutElsewhere && targetTiv !== undefined && targetPrem !== undefined && !alreadyThere) {
                ...
                const arrow = new THREE.ArrowHelper(to.clone().sub(from).normalize(), from, from.distanceTo(to), COLORS.redDeep, 4, 2.6);
                arrow.traverse((o) => {
                    o.raycast = () => undefined;
                });
                content.add(arrow);
                const changes = [];
                if (tivBand?.tier !== 'target')
                    changes.push(`TIV ${bandLabel(targetTiv)}`);
                if (premiumBand?.tier !== 'target')
                    changes.push(`premium ${bandLabel(targetPrem)}`);
            ...
            const cap = new THREE.Mesh(new THREE.SphereGeometry(2.4, 24, 16), new THREE.MeshStandardMaterial({
                color: account.verdict === 'REFER' ? COLORS.redTint : VERDICT_STYLES[account.verdict].fill,
                roughness: 0.5,
            }));
            ...
            content.add(cap);
            pickables.push({ mesh: cap, isPin: true, footprint: account });
            addLabel(`${account.insuredName} · no TIV or premium to stand on`, new THREE.Vector3(at.x + 16, stalkTop + 6, at.z), 24, COLORS.ink);
        }
    };
    ...
    const onMove = (ev) => {
        const node = pick(ev);
        if (hovered !== null && hovered !== node && hovered.isPin !== true)
        ...
        if (node.isPin !== true)
            node.mesh.scale.setScalar(node.footprint === undefined ? 1 : 1.4);
        const rect = host.getBoundingClientRect();
        const x = ev.clientX - rect.left;
        const y = ev.clientY - rect.top;
        if (node.cell !== undefined)
            callbacks.onHover({ kind: 'cell', cell: node.cell, x, y });
    ...
    const onUp = (ev) => {
        ...
        if (moved > 5)
            return;
        const node = pick(ev);
        if (node === null || node.footprint === undefined)
            return;
        if (node.isPin === true)
            callbacks.onOpen(node.footprint.submissionId);
        else
            callbacks.onSelect(node.footprint.submissionId);
    };
    const onLeave = () => {
        if (hovered !== null && hovered.isPin !== true)
            hovered.mesh.scale.setScalar(1);
        hovered = null;
        callbacks.onHover(null);
    };
 ...
 */
export function terrainAccountOf(detail) {
    const others = detail.factors.filter((f) => !SWEPT_FACTORS.has(f.factorId));
    const weightOf = (factorId, fallback) => detail.factors.find((f) => f.factorId === factorId)?.weight ?? fallback;
    return {
        ...
        refersElsewhere: others.some((f) => !f.known || f.tier === 'refer') ||
            detail.contradictions.some((c) => c.severity === 'HIGH' && c.status === 'open'),
    };
}
//# sourceMappingURL=explore-terrain.js.map