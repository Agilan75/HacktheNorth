...
afterEach(cleanup);
function renderHome() {
    render(_jsx(MemoryRouter, { initialEntries: ['/'], children: _jsx(HomePage, {}) }));
}
describe('HomePage', () => {
    it('leads with the wordmark and the thesis', () => {
        renderHome();
        expect(screen.getByRole('heading', { level: 1, name: 'Retrofit' })).toBeInTheDocument();
        expect(screen.getByText('Underwriting runs on what the broker typed. We built the camera that checks.')).toBeInTheDocument();
