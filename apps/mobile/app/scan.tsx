...
/**
 * The viewfinder, pushed from home once the room has been named. It is the
 * sweep: the phone takes the photos as you turn, and Finish is the only tap
 * between here and a price.
 *
 * This route hides its header, so the only way back is the close button drawn
 * over the camera and the quiet Back on each of the states that replace it.
 * Every one of them is load-bearing — without them a denied permission is a
 * full-screen dead end.
 *
...
export default function SweepScreen() {
  ...
  /**
   * Back to home, from any depth. `dismissTo` pops to home when it is already
   * in the stack and replaces this screen with it when it is not, so a cold
   * `retrofit://scan` link lands somewhere too. `replace('/')` would be wrong:
   * with home permanently underneath it swaps only the top frame and leaves two
   * home screens stacked on each other.
   */
  ...
  /**
   * The only way out of the live viewfinder, so it is a filled 44pt target
   * rather than a second bone link beside "Upload photos instead". Top right:
   * the HUD owns the top left with the coverage chip. Ink on bone is 15.35:1,
   * which holds over any camera image.
   */
  const closeButton = (
    <Pressable
      accessibilityRole="button"
      accessibilityLabel="Close"
      accessibilityHint="Leaves the scan and goes back. Nothing is sent."
      onPress={goHome}
      hitSlop={8}
  ...
  if (gate === 'checking') {
    return (
      <Screen edges={['top', 'bottom', 'left', 'right']} footer={<Button label="Back" variant="quiet" onPress={goHome} />}>
      ...
      <Screen
        ...
        footer={
          <>
            ...
            />
            <Button label="Back" variant="quiet" onPress={goHome} />
          </>
        }
      >
...
function SendView({
  ...
  if (send.kind === 'queued') {
    return (
      <Screen edges={['top', 'bottom', 'left', 'right']} title="No connection">
        ...
        {/* Safe to leave: the queue keeps sending, and the sweep registers itself on home. */}
  ...
  return (
    <Screen edges={['top', 'bottom', 'left', 'right']} title="Not sent">
...
const styles = StyleSheet.create({
  ...
  /**
   * The close target, pinned top right. The HUD owns the top left with the
   * coverage chip, so the two never collide. Filled rather than a bare glyph:
   * it sits on a live camera image, and ink on bone is 15.35:1 over anything.
   */
  close: {
    position: 'absolute',
    right: SPACE.lg,
