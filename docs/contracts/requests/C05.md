# C05 contract request — add `apps/console/src/pages/SubmissionPage.test.tsx` to C05's owns

**File:** `apps/console/src/pages/SubmissionPage.test.tsx` (new). Not in OWNERSHIP.json, so C05 did not create it.
**Change:** add it to C05's `owns` and commit the file below verbatim.
**Why:** the task requires a colocated test; the owns list does not allow creating one.
**Workaround used:** the identical test ran from the scratchpad with a throwaway vitest config (absolute imports) — 7 passed. The version below uses relative paths and runs under the normal `console` project: `VITEST_MAX_WORKERS=1 npx vitest run apps/console/src/pages/SubmissionPage.test.tsx --project console`.
