# C09 contract request — add the colocated panel test

**File to create:** `apps/console/src/panels/c09.test.tsx` (unowned; UNITS.json gives C09 only
`Buildings.tsx` and `Contradictions.tsx`, so C09 could not write its own colocated test).

**Change:** create the file with exactly the content below and assign it to C09 in OWNERSHIP.json.
It is purely additive; nothing else changes.

**Why:** the task requires a real colocated test; without this file the two panels have no test in
the default `console` project run.

**Workaround used:** the identical test (absolute imports instead of `./`) was run from C09's
scratchpad with a private vitest config (jsdom, repo `vitest.setup.ts`, automatic JSX):
`7 passed (7)`.
