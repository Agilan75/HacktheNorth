...
| ENV-2 | **Three engine/API tests fail on Windows and are left alone.** `data.test.ts` "resolves data beside src/" expects `/` separators; `fixtures/golden.test.ts` "byte-for-byte" expects LF and git checks out CRLF; `scripts/backfill.test.ts` reads `apps/api/data/backfill`, which was never committed: `git ls-files apps/api/data` is empty and nothing ignores it, so the 11 files it wants exist only on the machine that built them. | All three fail identically on `build/retrofit` with this branch stashed — verified. They are environment artefacts, not regressions, and all three sit in files this task may not touch. | Fixing them, which would mean editing frozen files for a problem this task did not create. |
...
## Phase C2 — the Viro implementation

| # | Decision | Why | Rejected alternative |
| --- | --- | --- | --- |
| C2-1 | **The choice between the two implementations is made on live data, not on the presence of the native module.** `useWorldPose` calls both hooks every render, in the same order, and returns the AR pose only while it has produced a camera transform in the last second. | The task requires that a failed Viro build still leaves a running app. Selecting on `hasViro()` alone would leave a development build frozen on a stale pose whenever the AR session failed to start, lost tracking, or landed on a device without ARCore. Selecting on freshness makes the fallback structural: it happens within a second, by itself, with nothing to remember to handle. | Choosing once at module load: fast, and wrong exactly when it matters. |
| C2-2 | **Nothing imports `@reactvision/react-viro` at module scope.** Every touch is a guarded `require` inside a function, and `hasViro()` rules Expo Go out first, by name, via `expo-constants`' `executionEnvironment === 'storeClient'`. | Metro bundles a statically imported module and evaluates it on load; ViroReact's JS reaches for a native module that Expo Go does not have. Asking "am I in Expo Go" before the `require` means the package is never even evaluated there. | A top-level import with a try/catch around its use: too late, the import has already run. |
| C2-3 | **`provider: "none"` in the config plugin.** | `withViro` defaults `provider` to `"reactvision"`, which then demands `rvApiKey` and `rvProjectId` and a ReactVision account. World tracking and plane detection are ARCore/ARKit features and need neither. | Leaving the default and adding an account to the build. |
| C2-4 | **One capture path for both.** `grab()` returns a file URI from whichever preview is live: Viro's `_takeScreenshot` or `expo-camera`'s `takePictureAsync`. Everything downstream is unchanged. | The AR session owns the camera when it runs, so `expo-camera` cannot also have it. Converging at the file URI means the downscale, the pricing call, the haptic and the capture machine are all identical on both paths, and only one of them can be wrong. | A second, parallel capture pipeline for AR. |
| C2-5 | **Plane detection changes the band's vertical extent, not its bearing.** `bandSpan` projects the wall's own height at its own distance; with no wall it is the fixed share of the viewport the gyro path uses. `planeForBearing` picks the wall the phone is pointing at, not the nearest one. | The task asks for the wash to paint on detected planes instead of at a fixed radius. Horizontal placement is already exact, because it comes from the same 10 degree panel grid the engine uses; what a fixed radius gets wrong is where the wall stops, which is what the plane fixes. Both functions live in `pose.ts` and are tested in node. | Rebuilding the whole wash as Viro scene geometry: it would need a second drawing path, and the overlay layer would stop being implementation-agnostic. |
| C2-6 | **The AR path is written but has never run.** No device here is authorised for debugging, there is no emulator image, and the only JDK present is newer than the Android Gradle Plugin accepts, so no development build could be produced. | Recorded rather than hidden. The Expo Go path is verified by `tsc`, 36 AR tests and `expo-doctor` 21/21; the AR path is verified by the same typecheck and by its store and geometry tests, and by the fallback in C2-1, which is what makes shipping it safe. Logged to `docs/status/blocked.jsonl`. | Claiming it works. |
...
| # | Decision | Why | Rejected alternative |
| --- | --- | --- | --- |
| D-1 | **Three registered faces, not two: `FONT_FAMILIES` gains `bodyStrong`.** | A phone cannot synthesise a weight for a custom font the way a browser can. Registering `Inter` alone and asking for `fontWeight: '600'` gets a fake bold on iOS and, below API 28, nothing at all on Android, and half this app's labels are semibold. Display text is always semibold in this system, so `display` is bound to Fraunces SemiBold and needs no second face. `typeStyle` still reads only the tokens and `grep -rn "fontFamily" apps/mobile` still returns two lines, both of them token references, with no literal and no `Platform.select` anywhere. | Two faces and faux-bold body text: worse type, for a word count. |
| D-2 | **The splash screen is held until the fonts load, but an error releases it.** | Painting in a fallback serif and then reflowing is worse than a beat of splash. Holding for ever because a font failed would be worse than either, so `useFonts`' error releases it and the platform falls back. | `if (!loaded) return null` with no splash control: a white flash. |
| D-3 | **The nav header's title style also reads `FONT_FAMILIES.display`.** | It is on-screen type. Leaving it out would put one heading per screen in the system font and quietly break "one type scale". | Leaving the header to React Navigation's defaults. |
| D-4 | **The verdict pill's shape glyph is a type step, not a pair of magic numbers.** It was `fontSize: large ? 17 : 15`. | Every size in the app comes from `FONT_SIZES` now, and the glyph has to grow with the words beside it or the pill breaks at large text sizes. | Keeping the literals. |
| D-5 | **The Brand wordmark uses the `heading` step instead of a size derived from the badge diameter.** | Same rule. It also means the wordmark scales with the user's text size, which a diameter-derived number never did. | `fontSize: Math.round(size * 0.5)`. |
...
| # | Decision | Why | Rejected alternative |
| --- | --- | --- | --- |
| E-1 | **The twelve old token names stay in `COLORS` as aliases, pointing at the new palette.** | The task's own instruction, and the alternative was editing the console, which rule 2 forbids. Keeping them inside `COLORS` rather than in a separate object is what lets `COLORS.mutedDeep` still compile, and lets the generated stylesheet keep every `--rf-*` the console's CSS already references. `MOBILE_COLOR_TOKENS` names the seven the phone app may use, and a test asserts every alias resolves to one of them. | A separate `LEGACY_COLORS` export: `COLORS.paper` would stop compiling, which is the thing the aliases exist to prevent. |
| E-2 | **`apps/console/src/styles/tokens.css` was regenerated. It is the only console file touched.** | Its own banner says "GENERATED … change packages/design/src/tokens.ts and regenerate", and a test exists to catch it going stale. Leaving it would have left the console's CSS naming colours its TypeScript no longer has. The consequence is real and is stated plainly: the console changes colour. Reverting that one file restores its old look at the cost of a mixed palette. | Leaving it stale. |
| E-3 | **Each verdict gets its own fill, and all three are filled.** FIT accent with ink (5.63:1), REFER amber with ink (7.27:1), DOES_NOT_FIT red with bone (4.76:1). | The new palette assigns amber to REFER and red to DOES_NOT_FIT by name, which reverses PRD §13's "red never means bad". Recorded as an override. REFER's old outlined form cannot survive the change: amber text on bone is 2.11:1. Every pill still carries its word and its shape mark, so colour still never carries meaning alone. | Keeping REFER outlined: unreadable. |
| E-4 | **`accent` on `bone` measures 2.73:1, not the ~3:1 the task expected**, so it is used for the price number only. Buttons and pills put ink on accent. | Measured, not assumed: `packages/design` now tests the number. At 2.73:1 accent is below AA-large as well as AA, so it cannot carry a button label. The price is the 34pt display step with the annual figure repeated in ink beneath it and the whole thing read out to a screen reader, which is why it is allowed there and nowhere else. Both exemptions are named in `CONTRAST_EXEMPTIONS` and asserted. | Bone on accent for buttons: 2.73:1, the same problem inverted. |
| E-5 | **A pressed primary button keeps its fill and takes an ink border.** | With one accent there is no darker accent to press to. A border is a shape, not a shade, so the pressed state does not depend on a colour difference at all. | A `muteTint` pressed state: the button would appear to lose its identity on touch. |
| E-6 | **`Card tone="accent"` is a bone fill with a 2px accent border, not an accent tint.** | There is no accent tint in a seven-colour palette, and filling with accent would put body text on 2.73:1. | Filling with `muteTint`: indistinguishable from `tone="muted"`. |
| E-7 | **`Hero` and `Badge` are deleted.** | `Hero` was a `LinearGradient`, and every gradient goes; its one caller is the share screen, which now leads with the price in accent on bone, as the verdict screen does. `Badge` had four tones needing four colours and, after Phase B, no caller outside the kit. | Keeping `Hero` as a flat fill: a large coloured panel is exactly the "gradient hero block" this rework removes. |
| E-8 | **`app.json` keeps two hex literals**, updated to `#F4EFE6` and `#D97757`. | It is Expo configuration read before any JavaScript runs; it cannot import a TypeScript token. Everything under `app/` and `src/` is clean: `grep -rnE "#[0-9A-Fa-f]{6}"` finds nothing there. | Leaving the old values: the splash and the Android accent would be the previous palette. |
| E-9 | **`VERDICT_STYLES` is explicitly typed instead of `as const`.** | With all three verdicts filled, `fill`'s literal union no longer includes `'transparent'`, so every `style.fill === 'transparent'` comparison became a type error — including one in `apps/console`, which may not be edited. Typing `fill` as `string` fixes both call sites without touching either. | A cast at each call site, one of which is in the console. |
| E-10 | **No dark mode was found to remove.** | `grep -rn "useColorScheme\|colorScheme"` over `app/` and `src/` returns nothing, and `app.json` already pins `userInterfaceStyle: "light"`. | — |

**Added to the overrides table:** OV-6 — PRD §13's "Red never means bad" no longer holds. The new
